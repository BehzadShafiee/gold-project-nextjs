...
router.get('/new-orders-list/:limit', async (req, res) => {
  try {
    ...
    // 2️⃣ Merge and sort
    const allOrders = [...sells, ...buys]
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
      .slice(0, limit);

    const orderIds = allOrders.map(o => o._id);

    // 3️⃣ Get ONLY related registers using $in
    const registers = await OrderRegister.find({
      orderId: { $in: orderIds }
    }).lean();

    // 4️⃣ Build register map
    const registerMap = {};
    registers.forEach(r => {
      registerMap[r.orderId.toString()] = r;
    });

    // 5️⃣ Collect all buy product ids
    const buyProductIds = allOrders
      .filter(o => o.order_type === 1)
      .map(o => o.selected_product);

    // 6️⃣ Fetch all products at once using $in
    const products = await Product.find({
      _id: { $in: buyProductIds }
    }).select("name").lean();
    ...
    const finalOrders = allOrders
      .map(order => {
        ...
        const registration = registerMap[order._id.toString()];

        let orderProduct = '';

        if (order.order_type === 1) {
          orderProduct = productMap[order.selected_product?.toString()] || '';
        } else {
          orderProduct = order?.product_name || '';
        }
      ...
      .filter(o => o.isRegister === 0); // filter unregistered
